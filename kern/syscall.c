...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if(curenv->env_type == ENV_TYPE_GUEST && e->env_ipc_dstva < (void*) UTOP) {
        ...
        struct VmxGuestInfo *ginfo;
        pte_t *gpte;
        void *hva, *gpa, *gva, *gpml4rt;
        pml4e_t *sptrt;
	    ...
	    envid_t guest, void* guest_pa, int perm)
{
    ...
    if (envid2env(srcenvid, &senv, 1) < 0 ||
        envid2env(guest, &genv, 1) < 0) return -E_BAD_ENV;

    if (srcva != ROUNDDOWN(srcva, PGSIZE) || guest_pa != ROUNDDOWN(guest_pa, PGSIZE))
        return -E_INVAL;
    if (srcva >= (void *) UTOP || guest_pa >= (void *)genv->env_vmxinfo.phys_sz)
        return -E_INVAL;
    if ((pp = page_lookup(senv->env_pml4e, srcva, &ppte)) == 0)
        return -E_INVAL;
    // XXX: check
    if (!perm || (perm & ~(__EPTE_READ | __EPTE_WRITE | __EPTE_EXEC)))
        return -E_INVAL;
    if ((perm & __EPTE_WRITE) && !(*ppte & PTE_W))
        return -E_INVAL;
    if ((res = ept_map_hva2gpa(genv->env_pml4e, (void *)page2kva(pp), guest_pa, perm, 0)) < 0) return res;
    else {
        pp->pp_ref++;
        return 0;
    }
