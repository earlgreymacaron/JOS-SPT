    ...
    while (level > 0) {
        ...
        if (!epte_present(dir[idx])) {
            if (!create) return -E_NO_ENT;
                ...
                dir[idx] = page2pa(p) | __EPTE_FULL;
                p->pp_ref++;
            }
        }
        dir = (epte_t *) KADDR(epte_addr(dir[idx]));
        level--;
    }
    if (epte_out) *epte_out = &dir[ADDR_TO_IDX(gpa, 0)];
    return 0;
}
...
int ept_page_insert(epte_t* eptrt, struct PageInfo* pp, void* gpa, int perm) {
    ...
    /* Your code here */
    pp->pp_ref++;
    return ept_map_hva2gpa(eptrt, page2kva(pp), gpa, perm, 1);
}
...
int ept_map_hva2gpa(epte_t* eptrt, void* hva, void* gpa, int perm, 
    ...
    epte_t *epte_out;
    int res;
    if ((res = ept_lookup_gpa(eptrt, gpa, 1, &epte_out)) < 0) {
        return res;
    }
    if (*epte_out != 0) {
